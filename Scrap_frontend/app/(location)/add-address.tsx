...
export default function AddAddressScreen() {
  ...
  const handleGetCurrentLocation = async () => {
    ...
    try {
      ...
      if (address) {
        setLocality(address.street || address.subregion || address.district || '');
  ...
  const handlePincodeBlur = async () => {
    ...
    try {
      ...
      if (!res.data.is_available) {
        setServiceError(`Sorry, we don't serve ${pincode} yet. Expanding soon!`);
      }
    } catch {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      await userService.addAddress({ type, flat_number: flat, locality, city, pincode });
      router.back();
    } catch (err: any) {
      const msg = err?.response?.data?.message || 'Could not save address. Please try again.';
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.chips}>
          {ADDRESS_TYPES.map((t) => (
            <Chip key={t} label={t.charAt(0).toUpperCase() + t.slice(1)} selected={type === t} onPress={() => setType(t)} />
          ))}
        </View>
        ...
        />
        <Input label="City" placeholder="e.g. Chandigarh" value={city} onChangeText={setCity} />
        <Input label="Pincode" placeholder="e.g. 160015" value={pincode} onChangeText={setPincode} keyboardType="number-pad" maxLength={6} onBlur={handlePincodeBlur} />
        {serviceError && (
      ...
      <View style={styles.cta}>
        <Button label="Save address details" onPress={handleSave} variant="primaryGreen" loading={loading} disabled={!!serviceError || checkingService} />
      </View>
    </SafeAreaView>
  );
}
