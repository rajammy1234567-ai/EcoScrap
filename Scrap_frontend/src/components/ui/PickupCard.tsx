...
export function PickupCard({ pickup, onPress }: Props) {
  ...
  const date = pickup.scheduled_at
    ? new Date(pickup.scheduled_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
    : new Date(pickup.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
  ...
  return (
    <Pressable style={styles.card} onPress={onPress}>
      ...
      <Text style={styles.items}>{pickup.items.length} item{pickup.items.length !== 1 ? 's' : ''}</Text>
...
const styles = StyleSheet.create({
  ...
  amount: { ...typography.body, fontWeight: '700', color: colors.neutral.black, marginTop: 4 },
});
